...
def _op_create_email_asset(req: dict) -> Tuple[int, dict]:
    brand = _normalize_brand(req)
    if brand != "Dodo":
        return 400, {"error": "Only brand=Dodo is supported.", "created": False, "warnings": ["Only brand=Dodo is supported."]}

    # IMPORTANT: folderPath is NOT supported here by design (use sfmc-folder-resolver tool first)
    if "folderPath" in req and str(req.get("folderPath") or "").strip():
        return 400, {
            "error": "folderPath is not supported in this tool.",
            "message": "Use sfmc-folder-resolver to get categoryId, then call createEmailAsset with categoryId.",
            "created": False,
            "warnings": ["folderPath not supported; use sfmc-folder-resolver then pass categoryId."],
        }

    if "categoryId" not in req:
        return 400, {
            "error": "Bad request",
            "message": "categoryId is required. Use sfmc-folder-resolver to obtain it.",
            "created": False,
            "warnings": ["categoryId is required."],
        }
    category_id = _clamp_int(req.get("categoryId"), 0, 0, 2_000_000_000)
    if category_id <= 0:
        return 400, {
            "error": "Bad request",
    ...
    if "preheader" not in req:
        return 400, {
            "error": "Bad request",
            "message": "preheader is required (can be empty string).",
            "created": False,
            "categoryId": category_id,
            "warnings": ["preheader is required (can be empty string)."],
        }
    preheader = (req.get("preheader") or "").strip()
    ...
    if not name or not subject:
        return 400, {
            ...
            "created": False,
            "categoryId": category_id,
            "warnings": ["name and subject are required."],
        }

    html = ""
    if req.get("htmlContentB64"):
        try:
            html = base64.b64decode(str(req["htmlContentB64"]).encode("utf-8")).decode("utf-8", errors="replace")
        except Exception:
            return 400, {
                "error": "Bad request",
                ...
                "created": False,
                "categoryId": category_id,
                "warnings": ["htmlContentB64 was not valid base64."],
            }
    else:
        html = (req.get("htmlContent") or "").strip()
    ...
    if not html:
        return 400, {
            ...
            "created": False,
            "categoryId": category_id,
            "warnings": ["htmlContent (or htmlContentB64) is required."],
        }
    ...
    try:
        ...
        return 200, {"created": True, "categoryId": category_id, "assetId": asset_id, "warnings": warnings}

    except Exception as e:
        logger.exception("CreateEmailAsset failed")
        return 502, {
            "error": "SFMC create failed",
            "errorType": type(e).__name__,
            "message": str(e),
            "created": False,
            "categoryId": category_id,
            "warnings": ["SFMC create failed", f"{type(e).__name__}: {str(e)}"],
        }
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if p == "/createemailasset":
            status, out = _op_create_email_asset(req)
            return respond(out, status)
